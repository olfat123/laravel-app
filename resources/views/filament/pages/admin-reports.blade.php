<x-filament-panels::page>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm">
            <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Commission Earned</p>
            <p class="text-2xl font-bold text-warning-600 dark:text-warning-400 mt-1">
                {{ number_format($summary['commission'], 2) }} EGP</p>
        </div>
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm">
            <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Vendor Payouts</p>
            <p class="text-2xl font-bold text-primary-600 dark:text-primary-400 mt-1">
                {{ number_format($summary['payout'], 2) }} EGP</p>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 shadow-sm mb-6">
        <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-4">Monthly Revenue Breakdown</h3>
        <div wire:ignore x-data="{
            ...
            init() {
                this.chart = new Chart($refs.canvas, {
                    ...
                    data: {
                        ...
                        datasets: [
                            { label: 'Revenue', data: {{ json_encode($chartData['revenue']) }}, backgroundColor: 'rgba(34,197,94,0.7)', borderRadius: 4 },
                            { label: 'Commission', data: {{ json_encode($chartData['commission']) }}, backgroundColor: 'rgba(234,179,8,0.7)', borderRadius: 4 },
                            { label: 'Vendor Payout', data: {{ json_encode($chartData['payout']) }}, backgroundColor: 'rgba(99,102,241,0.7)', borderRadius: 4 }
                        ]
                    },
            ...
            @update-admin-chart.window="
                ...
                chart.data.datasets[0].data = $event.detail.chartData.revenue;
                chart.data.datasets[1].data = $event.detail.chartData.commission;
                chart.data.datasets[2].data = $event.detail.chartData.payout;
                chart.update();
            ">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">
                                Commission</th>
                            <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">Payout
                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        @forelse($this->getMonthlyBreakdown() as $row)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                ...
                                <td class="px-4 py-2 text-right text-success-600 dark:text-success-400">
                                    {{ number_format($row['revenue'], 2) }}</td>
                                <td class="px-4 py-2 text-right text-warning-600 dark:text-warning-400">
                                    {{ number_format($row['commission'], 2) }}</td>
                                <td class="px-4 py-2 text-right text-primary-600 dark:text-primary-400">
                                    {{ number_format($row['vendor_payout'], 2) }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-6 text-center text-gray-400">No data</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">Qty
                                Sold</th>
                            <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">
                                Revenue</th>
                        </tr>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        @forelse($this->getTopProducts() as $i => $row)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                ...
                                <td class="px-4 py-2 text-gray-800 dark:text-gray-200 max-w-[180px] truncate">
                                    {{ $row['title'] }}</td>
                                <td class="px-4 py-2 text-right text-gray-600 dark:text-gray-400">{{ $row['qty'] }}
                                </td>
        ...
        <div
            class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100 dark:border-gray-700">
                <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-200">Top 10 Vendors by Revenue</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                        <tr>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">#</th>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Vendor
                            </th>
                            <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">Orders
                            </th>
                            <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        @forelse($this->getTopVendors() as $i => $row)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                <td class="px-4 py-2 text-gray-400">{{ $i + 1 }}</td>
                                <td class="px-4 py-2">
                                    <div class="font-medium text-gray-800 dark:text-gray-200">{{ $row['name'] }}</div>
                                    @if ($row['store_name'])
                                        <div class="text-xs text-gray-400">{{ $row['store_name'] }}</div>
                                    @endif
                                </td>
                                <td class="px-4 py-2 text-right text-gray-600 dark:text-gray-400">{{ $row['orders'] }}
                                </td>
